...
var vig = {
  ...
  add: function (options, app, handler) {
    if (httpMethods.indexOf(options.method) === -1) {
      console.error('HTTP Meold ' + options.method + ' is not valid!');
      return;
    }
    switch (options.action) {
      case 'routers':
        ...
        break;
      case 'policies':
        vig.addPoclies(options.method, options.url, handler);
        break;
      case 'validations':
      default:
  ...
  addPoclies: function (method, url, handler) {
    policies.push({
      url: url,
      method: method,
      handler: handler
